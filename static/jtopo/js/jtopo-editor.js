...
TopologyPanel.prototype.saveTopology = function (url) {
	...
	// var jsonParse = JSON.parse(topologyJSON);
	$.ajax({
		type: 'POST',
		url: '/back/asset/json_save',
		data: { jsondata: topologyJSON },
	...
	// var jsonParse = JSON.parse(topologyJSON);
	// console.log(JSON.parse(topologyJSON));
	// var eleLink = document.createElement('a');
	// eleLink.download = 'network.json';
	// eleLink.style.display = 'none';
	// console.log(eleLink);
	// // 字符内容转变成blob地址
	// var blob = new Blob([topologyJSON]);
	// console.log(blob);
 ...
 * 导出序列化的拓扑图JSON数据到服务器
 */
TopologyPanel.prototype.downTopology = function (url) {
	editor.mainMenu.hide();
	let self = this;
	// 保存container状态
	let containers = editor.utils.getContainers();
...
TopologyPanel.prototype.deleteAllNodes = function () {
	...
	if (editor.stage) {
		...
		// 	s.clear();
		// });
		// editor.stage.remove(this.scene);
		// 连线重置
...
TopologyEditor.prototype.initMenus = function () {
	...
	self.nodeMenu.on('mouseover', function (event) {
		...
		let text = $.trim($(event.target).text());
		let menuX =
			parseInt(this.style.left) + $(document.getElementById('change-node-text-pos')).width();
		// 边界判断
		...
		if (text === '文字位置') {
			...
			self.nodeTextPosMenu
				.css({
					top:
						parseInt(this.style.top) + $(document.getElementById('change-node-text-pos')).height(),
					left: menuX,
				})
				.show();
		} else if (text === '应用布局') {
	...
	self.nodeTextMenu.on('click', function (e) {
		...
		if ('修改节点文字' == text) {
			self.deviceEditText.val(editor.currentNode.text);
			// editor.utils.saveNodeInitState();
			//隐藏菜单显示文字输入框
			...
			self.deviceEditText
				...
				.show();
			self.deviceEditText.attr('value', editor.currentNode.text);
	...
	self.deviceEditText.blur(function () {
		if (self.currentNode) {
			self.currentNode.text = self.deviceEditText[0].value;
...
TopologyEditor.prototype.init = function (topologyGuid, backImg, topologyJson) {
	...
	this.scene.mouseover(function (event) {
		...
		if (event.target != null && event.target instanceof JTopo.Node) {
			...
			event.target != null &&
			event.target instanceof JTopo.Link &&
			event.target.linkTooltip &&
			editor.stageMode !== 'edit'
		) {
	...
	this.scene.mouseup(function (event) {
		...
		if (
			...
			event.target instanceof JTopo.Node &&
			event.target.layout &&
			event.target.layout.on &&
			event.target.layout.type &&
			event.target.layout.type !== 'auto'
		) {
	...
	this.stage.mouseout(function (event) {
		...
		if (
			self.link &&
			!self.isSelectedMode &&
			(event.target == null || event.target === self.beginNode || event.target === self.link)
		) {
			...
			if (datas && datas.length === 3) {
				...
				node.setBound(
					(event.layerX ? event.layerX : event.offsetX) -
						self.scene.translateX -
						self.config.nodeDefaultWidth / 2,
					(event.layerY ? event.layerY : event.offsetY) -
						self.scene.translateY -
						self.config.nodeDefaultHeight / 2,
					self.config.nodeDefaultWidth,
...
editor.utils = {
	...
	reMakeNodeAction: function () {
		if (
			editor.currentNode.currStep >= editor.currentNode.maxHistoryStep ||
